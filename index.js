...
function kopyala(copyTat) {
  let deneme = [...copyTat]; // spread operatoruyle kopyaladik
...
function dizi25Cesitmi(onayTat) {
  if (onayTat.length === 25) {
    return true;
  } else {
    return false;
  }
...
function cesitEkle(tatlarr, dizi) {
  tatlarr.unshift(dizi);
  return tatlarr;
}
...
function indekstekiCesitiGetir(cesit, dizi) {
  return cesit[dizi];
}
